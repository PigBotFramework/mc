...
class mc(PBF):
    ...
    def init(self):
        self.userItem = self.mysql.selectx("SELECT * FROM `botMC` WHERE `qn`=%s", (self.data.se.get("user_id")))
        if self.userItem:
            self.userItem = self.userItem[0]
            self.userItem['backpack'] = json.loads(self.userItem.get('backpack'))
    
    def die(self):
        self.mysql.commonx("DELETE FROM `botMC` WHERE `qn`=%s", (self.data.se.get("user_id")))
    ...
    def addLife(self, num=1):
        ...
        self.mysql.commonx("UPDATE `botMC` SET `life`=%s WHERE `qn`=%s", (self.userItem.get("life"), self.data.se.get('user_id')))
    
    def addXp(self, num=1):
        self.userItem['xp'] += num
        self.mysql.commonx("UPDATE `botMC` SET `xp`=%s WHERE `qn`=%s", (self.userItem.get("xp"), self.data.se.get('user_id')))
    ...
    def addBackpack(self, thi, num=1):
        ...
        self.mysql.commonx("UPDATE `botMC` SET `backpack`=%s WHERE `qn`=%s", (json.dumps(self.userItem.get("backpack"), ensure_ascii=False), self.data.se.get('user_id')))
    
    def addEvent(self, doing, utill):
        self.mysql.commonx("UPDATE `botMC` SET `doing`=%s, `doingutill`=%s WHERE `qn`=%s", (doing, utill, self.data.se.get('user_id')))
    ...
    def dig(self):
        ...
        self.client.msg().raw("[CQ:at,qq={0}] 开始挖矿\nface54当前时间：{1}\nface54挖矿时长：{2}秒\nface54获得矿物：{3}\nface54消耗{4}点饱食度\nface54获得{5}点经验\n请在{6}再回来吧！".format(self.data.se.get('user_id'), time.ctime(nowtime), rand, message, hungry, self.randomXp(), time.ctime(randtime)))
    ...
    def mobsComing(self):
        ...
        if self.userItem:
            if self.userItem.get("life") < 3:
                self.client.msg().raw("[CQ:at,qq={}] 遇到怪物，您的血量过低，被怪物撸死了！".format(self.data.se.get("user_id")))
                return self.die()
            ...
            if ob == 404:
                ...
                message = "[CQ:at,qq={}] 遇到怪物\nface54蜘蛛：{}\nface54小白：{}\nface54僵尸：{}\nface54爬行者：{}\n请选择武器反击（发送 不使用 可以纯手撸怪）".format(self.data.se.get("user_id"), spi, ski, bos, cre)
                self.client.msg().raw(message)
                self.commandListener.set("mc@mobsComing", args={"kill":spi+ski+bos+cre, "spi": spi, "ski": ski, "bos": bos, "cre": cre})
                return True
    ...
    def save(self):
        ...
        if self.userItem and self.userItem.get("hungry") >= 10:
            ...
            self.addLife(life)
            self.client.msg().raw("[CQ:at,qq={}] 回血{}滴，现有{}滴".format(self.data.se.get("user_id"), life, self.userItem.get("life")))
        else:
            return False
        return True
    ...
    def lifeToHungry(self):
        ...
        self.addHungry(hungry)
        self.addLife(-1*life)
        self.client.msg().raw("[CQ:at,qq={}] 已兑换！\nface54消耗{}滴血\nface54回了{}点饱食度".format(self.data.se.get("user_id"), life, hungry))
